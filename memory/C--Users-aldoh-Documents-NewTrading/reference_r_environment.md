...
**Renviron.site = R's site file (hard link).** `RApplication/Renviron.site` and `C:\Program Files\R\R-4.4.3\etc\Renviron.site` are the same file (same inode), so EVERY R session loads it, Rscript tasks from Task Scheduler included. It sets R_BOX_PATH, R_CONFIG_FILE, R_CONFIG_ACTIVE, R_DB_PATH, R_LOG_DIR, RETICULATE_PYTHON and ALERT_EMAIL_PASSWORD. These are NOT Windows environment variables: PowerShell `[Environment]::GetEnvironmentVariable` and `.Renviron` searches come back empty. Check from R (`Rscript -e 'nzchar(Sys.getenv("X"))'`). On 2026-09-23 I wrongly told the user email was not configured after checking only the OS. A new R version would need the link re-created.
