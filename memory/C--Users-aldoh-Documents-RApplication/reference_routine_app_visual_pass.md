...
Caveat: **Gonet account loads fine headless** (reads the stored DB snapshot). **IBKR accounts (U1804173 etc.) HANG** — the account module fetches live data and `set_inputs` times out at ~90s ("Server did not update any output values within 90 seconds"). So validate shared display logic on Gonet; for IBKR-only columns drive the render functions directly on a stored `readPortfolio()` snapshot instead (no app, no TWS — see [[reference_dt_idioms]]). Pre-existing boot warnings (globals 'primer'; duplicate ids `displaysym-symbolsPortfolio`/`getPortfolio-timePortfolio`) are harmless. Each AppDriver run also churns `renv/activate.R` — revert it, [[reference_renv_discipline]]. Related: [[feedback_shinytest2_for_silent_bugs]].
